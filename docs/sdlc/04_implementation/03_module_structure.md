...
+-----------------------------------+
                  ...
                  v
+===================================+
|    Module Structure v1.0 [DOK]    |  <-- POSISI DELIVERABLE INI
+===================================+
                  |
                  v
+-----------------------------------+
...
*   **Dokumen Input (Acuan Utama)**:
    - [Software Requirements Specification v1.1](docs/sdlc/02_analysis/02_software_requirements.md): Sumber 10 modul, 40+ kebutuhan fungsional (SRS-F-001 s.d SRS-F-040), dan standard error codes.
...
| **M.3** | digital & Servis | Layanan deposit & saldo PPOB, optimalisasi biaya 6 e-wallet, pendaftaran & status perbaikan unit service pelanggan. | pramuniaga, kasir, desainer, pemilik |
...
| **M.8** | CRM Pelanggan | Pengelolaan profil keanggotaan pelanggan (enkripsi WhatsApp mematuhi UU PDP No. 27/2022). | pramuniaga, kasir, kepala_percetakan, pemilik |
...
*   `db/`: Inisialisasi pool koneksi biner MySQL, retry mechanism dengan exponential backoff harian, dan transactional block ACID.
...
Layer ini diperbolehkan memiliki efek samping I/O (seperti `print()` dan `input()`), namun dilarang keras melakukan perhitungan matematika langsung atau membangun string query SQL.
...
*   **Tabel Database diakses**: `pengguna`, `transaksi`, `antrian_kerja` (via `db.query_builder`).
*   **Hak Akses / Peran**: Seluruh 8 peran terdaftar.
...
*   **Dependensi Impor**: `logic.safety_validator`, `db.query_builder`, `middleware.rbac_guard`, `utils.text_formatter`.
...
*   **Hak Akses / Peran**: `pemilik`, `kepala_percetakan`, `pramuniaga`, `kasir`, `fotocopy_print` (retail eceran terbatas). `desainer`, `produksi_cetak`, dan `gudang` ditolak (DENY).

### 4.4. File: cli/menu_inventaris.py
*   **Tujuan/Tanggung Jawab**: Menyediakan panel input master barang desimal, formula BOM kustom, Stock Opname fisik, prediksi stok, price tracking supplier, import CSV Excel, dan backup manual.
*   **Modul Fungsional**: `M.2` Inventaris, BOM & Stock Opname, `M.5` Antrian & Pelacakan Desain.
*   **Daftar Fungsi Publik**:
    -   `show_menu_inventaris(session_state: dict) -> None`: Menampilkan menu Modul 2.
...
*   **Hubungan Use Case / Menu ID**: `MENU-M2-001` s.d `MENU-M2-010`, `MENU-M5-001`, `MENU-M5-002`.
*   **Tabel Database diakses**: `barang`, `bom_komposisi`, `stock_opname`, `limbah_produksi`, `supplier`, `backup_logs` (via Layer 3).
*   **Hak Akses / Peran**: `pemilik`, `kepala_percetakan`, `gudang` (akses penuh master/opname), `produksi_cetak` (HPP & limbah), `desainer` (antrian). `pramuniaga`, `kasir`, dan `fotocopy_print` ditolak.
...
*   **Hak Akses / Peran**: `pemilik`, `kepala_percetakan`, `pramuniaga`, `kasir`, `fotocopy_print`.
...
*   **Tabel Database diakses**: `absensi`, `kasbon`, `payroll`, `pinjaman_bank`, `pinjaman_kerabat`, `pengeluaran`, `aset` (via Layer 3).
*   **Hak Akses / Peran**: `pemilik` (Akses Penuh), `kepala_percetakan` (verifikasi absensi), `kasir` (input opex kecil). Aktor level 3 lainnya hanya `📝 INPUT` absensi diri sendiri.
...
*   **Hak Akses / Peran**: `pemilik` (Absolute Lockdown). Staf lain ditolak (DENY).
...
*   **Aturan FP Murni**:
    -   Variabel input dibungkus imutabel NamedTuple `BOMKomponen`, `KalkulasiResult`, dan `ProcessResult`.
...
*   **Tujuan/Tanggung Jawab**: Komputasi gaji bulanan cerdas berbasis bagi hasil 25% target laba toko (jika laba < Rp 15.000.000), proteksi UMR minimum (50% UMR default Rp 1.600.000), perhitungan komisi insentif poin staf (4-tier), dan auto-cut sisa utang kasbon bulanan.
...
*   **Tujuan/Tanggung Jawab**: Kalkulasi depresiasi penyusutan garis lurus aset bulanan, tabungan virtual mesin baru, amortisasi pinjaman bank komersial & kerabat, rekap laporan laba/rugi instan divisi, dan proteksi threshold limit pengeluaran opex kasir.
...
*   **Contoh Signature Fungsi**:
    ...
    def sanitasi_input_cli(raw_input: str) -> str:
        """Membuang biner kontrol ASCII di bawah byte 0x20."""
        pass

    def validasi_kekuatan_sandi(password: str) -> ValidationStatus:
        """Validasi biner kriteria sandi aman."""
        pass
    ```
...
Layer ini bertanggung jawab penuh mengelola connection pooling, retry mechanism dengan exponential backoff harian, dan transactional block ACID.
...
*   **Tujuan/Tanggung Jawab**: Inisialisasi local database connection pool bawaan driver (`pool_name="abupool"`, `pool_size=5`), pencegahan kebocoran koneksi, implementasi retry automatic connection dengan exponential backoff 3x harian saat menangkap MySQL error `2006` (gone away) dan `2013` (lost connection).
*   **Modul Fungsional**: `M.7` Keamanan & Hak Akses.
*   **Pola Pooling & Retry**:
...
*   **Tujuan/Tanggung Jawab**: Wrapper eksekusi query SQL aman terparameter biner (`%s` bindings) untuk mencegah SQL injection, dan block penjamin atomik ACID transaksi.
...
*   **Tujuan/Tanggung Jawab**: Enkripsi sandi Blowfish `bcrypt` dengan dynamic salt Cost Factor = 12, otentikasi username, pembuatan dan verifikasi biner token stateless session JWT signature `HS256` masa aktif 8 jam biner, suspensi rate limiting lockout 10 menit setelah 5x gagal login berturut-turut.
...
*   **Tujuan/Tanggung Jawab**: Interceptor fungsional otorisasi level menu terminal CLI mencocokkan peran JWT aktif dengan Access Control Matrix (8 peran internal), merekam insiden `ACCESS_DENIED` secara biner ke log audit database.
*   **Modul Fungsional**: `M.7` Keamanan, Audit Trail & Hak Akses.
*   **Contoh Signature Fungsi**:
    ```python
...
*   **Tujuan/Tanggung Jawab**: Pencatatan modifikasi data sensitif (stok manual, opex > Rp 500rb, login failed) ke dalam database MySQL tabel `audit_logs` format terstruktur JSON string untuk merekam `old_value` dan `new_value`.
*   **Modul Fungsional**: `M.7` Keamanan, Audit Trail & Hak Akses.
*   **Contoh Signature Fungsi**:
    ```python
    def log_audit_trail(pengguna_id: int, action_type: str, target_table: str, old_val: dict | None, new_val: dict | None, cabang_id: int, db_connection) -> None:
        """Menyimpan berkas log audit terstruktur JSON biner ke MySQL."""
        pass
    ```
...
*   **Tujuan/Tanggung Jawab**: Mengunci secara biner versi pustaka pihak ketiga wajib target AbuCom:
...
*   **Tujuan/Tanggung Jawab**: Integration testing sandbox memanfaatkan database testing `abucom_test_db` untuk mensimulasikan login multi-user dan memvalidasi lemparan status otorisasi illegal `ACCESS_DENIED`.
...
| **M.3 — digital & Jasa Service**| `menu_ppob_service.py`| `safety_validator.py` | `query_builder.py` | `audit_logger.py` | `text_formatter.py` |
...
| **SRS-F-011** | Rekonsiliasi Stock Opname | `cli/menu_inventaris.py` | `form_stock_opname()` |
...
| **SRS-F-022** | Job Tracking Antrian | `cli/menu_inventaris.py` | `form_stock_opname()` |
| **SRS-F-023** | Arsip Desain Pelanggan | `cli/menu_inventaris.py` | `form_kelola_barang()` |
| **SRS-F-024** | Notifikasi WhatsApp Web | `cli/menu_inventaris.py` | `form_stock_opname()` |
...
| **SRS-F-027** | Alert Jatuh Tempo H-3 | `cli/menu_sdm_finansial.py` | `form_pinjaman_modal()` |
...
| **SRS-F-040** | Supplier & Utang Usaha | `cli/menu_inventaris.py` | `form_kelola_barang()` |
...
| 25| `stock_opname` | `query_builder.py` | `bom_hpp.py` |
| 26| `riwayat_harga_supplier` | `query_builder.py` | `bom_hpp.py` |
| 27| `audit_logs` | `query_builder.py` | `safety_validator.py` |
| 28| `backup_logs` | `query_builder.py` | `bom_hpp.py` |
