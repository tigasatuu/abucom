...
status     : Final
dibuat     : 2026-05-25
penyusun   : Senior Manufacturing Systems Architect & Cost Accounting Specialist
---
...
+-----------------------------------+
                  ...
                  v
+===================================+
|   BOM & HPP Design v1.0 [DOK]     |  <-- POSISI DOKUMEN INI
+===================================+
                  |
                  v
+-----------------------------------+
...
*   **Dokumen Input (Acuan Utama)**:
    ...
    - [System Architecture](docs/sdlc/03_design/03_system_architecture.md): Panduan paradigma FP murni dan State Dictionary Passing.
...
*   *Analogi Sederhana*: Sebuah stempel flash bulat bukan sekadar satu barang utuh. Di balik layar, ia terbentuk dari gabungan: gagang stempel flash (1 unit), karet flash bulat (dimensi panjang $\times$ lebar desimal), tinta stempel khusus (volume mililiter desimal), dan kertas buffalo cover (lembaran).
...
Diagram Mermaid ERD di bawah memetakan hubungan relasional Crow's Foot di antara entitas database yang secara langsung berpartisipasi dalam pemrosesan BOM, perhitungan HPP, pencatatan limbah, dan pembukuan pengeluaran:
...
### 3.3. DDL Tabel `barang` (Kolom Relevan BOM)
Kolom-kolom di dalam tabel `barang` yang berinteraksi langsung dalam kalkulasi BOM/HPP:
```sql
CREATE TABLE barang (
    id INT NOT NULL AUTO_INCREMENT PRIMARY KEY,
    nama_barang VARCHAR(100) NOT NULL,
    tipe_barang VARCHAR(20) NOT NULL, -- 'Retail_ATK' | 'Bahan_Baku'
    satuan_uom VARCHAR(20) NOT NULL,
    ...
    -- ... kolom lain didefinisikan lengkap di 01_database_schema.sql
    CONSTRAINT chk_barang_harga_beli CHECK (harga_beli >= 0),
    CONSTRAINT chk_barang_harga_retail CHECK (harga_retail >= 0)
);
```
*   `stok_saat_ini`: Dideklarasikan sebagai `DECIMAL(15,4)` untuk mendukung kuantitas pecahan desimal presisi tinggi dari sisa bahan lembaran atau cairan.
*   `harga_beli`: Harga per unit bahan dari supplier yang menjadi basis penentuan HPP komponen.
...
1.  **Karet Flash Bulat (Bahan Baku)**:
    *   Ukuran pemakaian: $0.05 \text{ m} \times 0.05 \text{ m} = 0.0025 \text{ m}^2$.
    *   Harga Beli Supplier: Rp 100.000 / $\text{m}^2$.
    *   *Biaya Karet*: $0.0025 \times 100.000 = \text{Rp } 250,0000$.
2.  **Gagang Stempel Flash Bulat (Bahan Baku)**:
    *   Kuantitas pemakaian: $1.0000 \text{ Pcs}$.
    *   Harga Beli Supplier: Rp 4.500 / Pcs.
    *   *Biaya Gagang*: $1.0000 \times 4.500 = \text{Rp } 4.500,0000$.
...
Kalkulasi biaya pembuatan 1 unit Buku Yasin Hardcover kustom:
1.  **Cover Yasin Hardcover (Bahan Baku)**:
    *   Pemakaian: $1.0000 \text{ Pcs}$.
    *   Harga Beli: Rp 5.000 / Pcs.
    *   *Biaya Cover*: $1.0000 \times 5.000 = \text{Rp } 5.000,0000$.
2.  **Kertas Matt Paper Isi Yasin (Bahan Baku)**:
    *   Pemakaian: $100.0000 \text{ Lembar}$ (atau setara $0.2000 \text{ Rim}$).
    *   Harga Beli Satuan: Rp 100 / Lembar.
    *   *Biaya Kertas*: $100.0000 \times 100 = \text{Rp } 10.000,0000$.
3.  **Tinta Pigment Art Paper (Retail ATK diambil Produksi)**:
    *   Pemakaian: $0.0100 \text{ Liter}$ ($10 \text{ Ml}$).
    *   Harga Beli Satuan: Rp 150.000 / Liter.
    *   *Biaya Tinta*: $0.0100 \times 150.000 = \text{Rp } 1.500,0000$.
4.  **Lem Buku / Hot Melt (Bahan Baku)**:
    *   Pemakaian: $0.0500 \text{ Pcs}$.
    *   Harga Beli Satuan: Rp 20.000 / Pcs.
    *   *Biaya Lem*: $0.0500 \times 20.000 = \text{Rp } 1.000,0000$.

Kalkulasi HPP Total Produk Buku Yasin:
$$\text{HPP Total} = \text{Rp } 5.000 + \text{Rp } 10.000 + \text{Rp } 1.500 + \text{Rp } 1.000 = \text{Rp } 17.500,0000$$
...
flowchart TD
    ...
    B -->|Ya| D[INSERT ke bom_komposisi]
    
    D --> E[Antrian Kerja Kustom: Masuk Status 'Selesai']
    E --> F[Sistem: Auto-Fetch Formula bom_komposisi]
    F --> G[Sistem: Ambil Harga Beli Terkini di barang]
    G --> H[Sistem: Hitung HPP Total via decimal.Decimal]
    
    H --> I[Sistem: UPDATE subtotal & HPP detail_transaksi]
    I --> J[START TRANSACTION]
    J --> K[UPDATE barang: Potong stok_saat_ini]
    ...
    Q --> R[UPDATE barang: Potong stok_saat_ini & INSERT pengeluaran OPEX]
    P -->|Tidak| S[Selesai]
    R --> S
...
*   **Alur Prosedural**:
    ...
    3.  Sistem memverifikasi `uq_bom_komposisi_induk_bahan`.
...
### 5.3. Alur 2 — Kalkulasi HPP saat Produksi Selesai (Trigger Utama)
...
sequenceDiagram
    ...
    Logic->>DB: SELECT harga_beli FROM barang WHERE id = %s
    ...
    CLI->>DB: UPDATE barang SET stok_saat_ini = stok_saat_ini - %s WHERE id = %s (Potong Stok Bahan)
    CLI->>DB: COMMIT TRANSACTION
    DB-->>CLI: OK / Commit Sukses
    CLI-->>Staf: Tampilkan alert sukses hijau & update stok berhasil
```

### 5.4. Alur 3 — Pemotongan Stok Bahan Baku Desimal
...
4.  Nilai tersebut dibukukan sebagai pengeluaran operasional internal di tabel `pengeluaran`.
...
|---|---|---|---|
| `kuantitas_desimal` | `DECIMAL(15,4)` | Harus berupa angka numerik positif > 0. | Lempar `ERR-VAL-007`, kembalikan prompt. |
...
1.  **Integrasi ke M.1 Transaksi & Nota**:
    Saat transaksi belanja dibuat, produk cetak kustom yang masuk ke dalam keranjang belanja `detail_transaksi` mengunci nilai subtotal awal. Begitu status antrian produksi selesai, nilai HPP riil yang baru dikalkulasi disimpan secara permanen di baris transaksi detail bersangkutan.
...
5.  **Integrasi ke M.7 Audit Trail**:
    Setiap manipulasi data formula BOM (tambah/hapus) atau eksekusi pemotongan stok desimal direkam secara kronologis ke dalam tabel `audit_logs` dengan format JSON yang mencakup salinan rekaman lama (`old_value`) dan rekaman baru (`new_value`).
...
### 8.2. Penerapan Paradigma FP Murni
...
Berikut adalah pseudocode implementasi fungsi murni kalkulasi dan pemrosesan transaksional BOM & HPP:
...
def proses_pemotongan_stok(komponen_list: list[BOMKomponen], cabang_id: int, db_connection) -> ProcessResult:
    ...
    try:
        ...
        for comp in komponen_list:
            # 1. Tarik stok saat ini dengan query terisolasi (Lock row)
            cursor.execute(
...
def proses_limbah_produksi(
    ...
    try:
        ...
        # 1. Query harga beli terkini bahan baku
        cursor.execute(
...
| **UC-010** | SRS-F-010 | WF-M2-04 (ATK Internal) | `MENU-M2-004` (Sinkronisasi ATK) | Bagian 2.4, Bagian 5.6 |
