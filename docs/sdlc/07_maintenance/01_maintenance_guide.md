...
|:---:|:---:|---|---|
| **1.1** | 2026-05-27 | Validasi menyeluruh dan penyempurnaan dokumen menjadi versi v1.1. Penerapan pengamanan skrip cron backup (membaca .env), penambahan sistem laci kasir RJ11 pada SOP Mingguan, sinkronisasi kontak eskalasi darurat, inventarisasi router MikroTik, penambahan 3 kasus troubleshooting (ERR-AUTH-003, ERR-AUTH-011, ERR-AUTH-029), penambahan glosarium CLI dan RBAC, penambahan templat visual output (Logbook, Incident, Patch), penggantian kata hubung "and" asing menjadi "dan" secara masif, serta penyelarasan komentar OS pada seluruh blok kode. | Senior IT Service Manager & Maintenance Operations Architect |
...
### 1.7. Templat Format Output
Untuk menunjang kelancaran operasional pemeliharaan harian, berikut adalah contoh format minimal (actionable templates) output logbook, laporan insiden, dan catatan rilis patch yang harus dicatat atau dicetak luring.

#### 1.7.1. Maintenance Logbook (Harian/Mingguan/Bulanan)
```text
+------------------------------------------------------------------------------------+
|                               ABUCOM — MAINTENANCE LOGBOOK                         |
+------------------------------------------------------------------------------------+
| Hari/Tanggal  : [TANGGAL]                                                          |
| Petugas       : [NAMA PETUGAS]             Peran: [PERAN]                          |
+------------------------------------------------------------------------------------+
| [ ] Pemeriksaan UPS 1 & 2 (Hijau/Alarm)     | Status: [OK / ALARM]                 |
| [ ] Booting Server & Kasir (Sukses/Eror)    | Status: [OK / EROR]                  |
| [ ] Input Kas Awal Laci                     | Status: [SUKSES / GAGAL]             |
| [ ] Log Backup Harian Otomatis Semalam      | Status: [SUCCESS / ERROR]            |
| [ ] Dashboard Pemilik (Bebas Anomali)       | Status: [AMAN / FRAUD DETECTION]     |
| [ ] Serah Terima Shift Kasir (Selisih)      | Nominal Selisih: Rp [............]  |
| [ ] Graceful Shutdown Server Debian         | Jam: [........] WIB                  |
+------------------------------------------------------------------------------------+
| Catatan Khusus & Tindakan:                                                         |
|                                                                                    |
|                                                                                    |
+------------------------------------------------------------------------------------+
```

#### 1.7.2. Incident Report Form (Laporan Insiden)
```text
+------------------------------------------------------------------------------------+
|                               ABUCOM — INCIDENT REPORT                             |
+------------------------------------------------------------------------------------+
| ID Insiden    : INC-YYYYMMDD-[NO]          Status: [OPEN / RESOLVED]               |
| Dilaporkan Oleh: [NAMA PELAPOR]            Tanggal: [TANGGAL]  Jam: [.....] WIB    |
| Severity Level: [S1 / S2 / S3 / S4 / S5]   Priority: [P1 / P2 / P3 / P4]           |
+------------------------------------------------------------------------------------+
| Gejala Teknis / Kode Error:                                                        |
| [ ] ERR-DB (Database)    [ ] ERR-AUTH (Autentikasi)    [ ] ERR-SESSION (Sesi)      |
| [ ] ERR-FILE (Backup)    [ ] ERR-CASH (Laci Kasir)     [ ] Visual/Printer/Lainnya  |
| Detail Gejala:                                                                     |
|                                                                                    |
+------------------------------------------------------------------------------------+
| Langkah Penanganan Sementara (Isolasi):                                            |
|                                                                                    |
|                                                                                    |
+------------------------------------------------------------------------------------+
| Solusi Akhir & Pencegahan (Root Cause):                                            |
|                                                                                    |
+------------------------------------------------------------------------------------+
| Tanda Tangan Pelapor:                      Tanda Tangan Supervisor:                |
|                                                                                    |
| (..................)                       (..................)                    |
+------------------------------------------------------------------------------------+
```

#### 1.7.3. Patch Release Notes (Catatan Rilis Patch)
```text
+------------------------------------------------------------------------------------+
|                               ABUCOM — PATCH RELEASE NOTES                         |
+------------------------------------------------------------------------------------+
| Versi Patch   : v[MAJOR].[MINOR].[PATCH]   Tanggal Rilis: [TANGGAL]                |
| Dibuat Oleh   : [NAMA PENGEMBANG]          Base Branch  : [main / patch / hotfix]  |
+------------------------------------------------------------------------------------+
| 1. DAFTAR PERBAIKAN BUG / KOREKSI KODE (BUG FIX LIST)                              |
|    - ID Bug: [e.g. DEF-COMPAT-001]                                                 |
|      Akar Masalah: [Penjelasan singkat akar masalah teknis]                        |
|      Solusi Kode : [Penjelasan file & perubahan logika]                             |
|                                                                                    |
| 2. PUSTAKA DEPENDENSI (MANDATORY WHEELS UPDATED)                                   |
|    - [Pustaka==Versi]                                                              |
|                                                                                    |
| 3. VERIFIKASI PENGUJIAN (TEST SUITE RESULTS)                                       |
|    - pytest pass rate: [100%]              Coverage logic: [.......%]              |
|    - Smoke Test ST-01 s.d ST-06 Status   : [PASS / FAIL]                           |
+------------------------------------------------------------------------------------+
```
...
2.  **M.2 — Manajemen Inventaris, BOM & Stock Opname**: Formula HPP produk cetak berbasis BOM desimal, limbah (*waste*), penguncian opname, price tracking supplier, import/export CSV, dan backup-restore manual.
...
5.  **M.5 — Sistem Manajemen Antrian & Pelacakan Desain**: Antrian job 5 status, rekam path PDF mockup desain, dan link template WhatsApp Web.
6.  **M.6 — Administrasi Pinjaman, Aset & Pengeluaran**: Pengeluaran operasional (biaya &ge; Rp 500.000 butuh sandi Pemilik), depresiasi aset garis lurus, tabungan aset virtual, pinjaman bank/kerabat, dan laporan laba rugi.
7.  **M.7 — Keamanan, Audit Trail & Hak Akses**: Autentikasi bcrypt cost 12, sesi JWT 8 jam, rate limiting login 5x, validasi RBAC, shift handover, dan audit logs JSON.
8.  **M.8 — Pembatalan, Retur & CRM**: Otorisasi pembatalan/retur (butuh sandi Pemilik), profile CRM pelanggan, dan hard delete UU PDP.
...
*   **Contoh Kasus Spesifik AbuCom**:
    *   Perbaikan bug `DEF-COMPAT-001` (mojibake ANSI visual garis tabel di terminal kasir CMD CP1252 lawas).
...
2.  **Nyalakan Server**: Tekan tombol power Mini PC Server, tunggu &plusmn; 2 menit hingga system booting CLI Debian aktif di layar console.
...
5.  **Shutdown Server (Pukul 21:05 WIB)**: Setelah memastikan backup harian sukses (verifikasi lewat status `[SUCCESS]` di `/var/log/abucom_backup.log` via SSH terminal kasir), administrator login SSH server, jalankan perintah graceful shutdown:
    ```bash
...
2.  Kasir menghitung uang kertas dan koin fisik di laci kas dan menginputkan nominalnya (`kas_fisik`).
...
2.  Sediakan minimal 2 gulungan cadangan kertas thermal nota lebar 58mm/80mm di laci konter kasir.

### 5.6. SOP Verifikasi Pemicu Elektrik Laci Kasir (RJ11)
1.  Hubungkan unit laci kasir RJ11 ke port belakang printer thermal nota.
2.  Jalankan cetak struk kosong atau transaksi uji coba luring Smoke Test (ST-01) dari terminal kasir.
3.  Pastikan laci kasir terbuka secara elektrik secara otomatis, cepat, dan lancar tanpa tersangkut.
4.  Bersihkan rel laci kasir dari debu kertas atau selipan koin fisik yang berpotensi menghalangi buka-tutup laci.

### 5.7. Checklist Pemeliharaan Mingguan
...
- [ ] 7. Pemicu elektrik laci kasir RJ11 teruji terbuka otomatis dan lancar.
...
2.  Periksa kapasitas penyimpanan SSD server:
    ```bash
    # [LINUX DEBIAN 12 — Server]
    df -h
    ```
...
- [ ] 5. Pengujian pelepasan catu daya UPS sukses menyangga beban server dan Klien minimal 5 menit.
...
4.  Lakukan kueri pencocokan nominal kas harian dan stok bahan baku desimal untuk memastikan data restore 100% konsisten.
...
1.  Uji dekripsi berkas ZIP backup manual menggunakan utilitas terminal:
    ...
    unzip -t -P AbuCom_SecureBackupZip_Pass_2026_X9z! backup_latest.zip
    ```
...
1.  Review rilis roadmap AbuCom v1.1.0 dan v1.2.0 (Bab 5 roadmap Release Notes R-05).
...
1.  **Pelaporan Awal**: Kasir mencatat gejala eror, screenshot layar terminal, dan catat kode error (`ERR-XXX-XXX`).
...
3.  Pastikan akar masalah diidentifikasi secara atomik (apakah masalah database constraint, runtime Dual-OS path, atau mathematical rounding).
...
1.  **Branching Git**: Buat branch terisolasi dari base `main` menggunakan Git Workflow (R-10):
    ...
    ```
2.  **Coding Standard**: Tulis perbaikan kode 100% mematuhi Coding Standard FP Python murni (R-09) tanpa kelas, no global state mutation, dan explicit UTF-8 parameters.
...
|---|---|---|---|
| **S1 (Blocker)** | &le; 15 Menit | &le; 2 Jam | Hentikan kasir, isolasi, picu rollback instan atau hotfix darurat. |
...
5.  Duplikat berkas `.env` produksi, sesuaikan parameter `APP_CABANG_ID` dan `PRINTER_PORT` kasir 2.
...
- [ ] 4. (Post) Interpreter Python baru dan dependensi sukses dimuat di virtual environment.
...
3.  Subprocess Python secara otomatis mengeksekusi dump database lokal, merespon status sukses, dan menyalin file ZIP AES-256 terenkripsi di folder `C:\exports\backups\`.
...
Jika Mini PC Server database terbakar, dicuri, atau rusak total harddisk fisiknya:
1.  **Pengadaan Hardware Baru**: Siapkan Mini PC pengganti dengan spesifikasi standard Intel i5, RAM 16GB, SSD 512GB NVMe (Sesuai Bab 16).
2.  **Instalasi Base OS**: Lakukan setup Debian 12 minimal CLI, pasang interpreter Python 3.14.2+, dan pasang MySQL Server 8.4 LTS (Bab 11).
3.  **Ekstraksi Berkas Cadangan**: Salin berkas ZIP backup manual terenkripsi terakhir (yang disimpan offline di PC Kasir atau external HDD Pemilik) ke server baru di `/tmp/restore.zip`.
...
*   **Otomatis**: Sistem memonitor attempts failed login, pemicuan menu oleh peran ilegal, atau nominal pengeluaran di atas threshold Rp 500.000.
*   **Manual**: Staf kasir atau gudang mendeteksi malfungsi peripheral fisik printer thermal atau colokan LAN switch hub mati.

### 13.4. Prosedur Respon Insiden (Isolasi, Pelaporan, Pemulihan)
1.  **Isolasi (Containment)**: Hentikan eksekusi transaksi pada terminal yang bermasalah. Cabut token JWT user aktif. Putuskan kabel LAN node bermasalah jika dicurigai infeksi virus lokal.
2.  **Pelaporan (Reporting)**: Catat gejala teknis, kode error, dan pemicu insiden. Laporkan tiket insiden formal kepada Administrator.
3.  **Pemulihan (Recovery)**: Administrator login SSH untuk menelisik log audit MySQL, restore data aman dari backup ZIP, atau update file konfig `.env`.
...
1.  **Daya Penyangga**: Alarm unit UPS 1 dan 2 berbunyi beep berkala menandakan daya baterai aktif (mampu menyangga s.d 15 menit).
...
3.  **Fase Shutdown Klien**: Kasir menutup program CLI kasir secara normal, mematikan OS Windows 11 PC kasir secara aman, dan menekan tombol power off unit UPS kasir.
...
| Database Password | MySQL Server / `.env` | 6 Bulan Sekali | SysAdmin | Query `ALTER USER` dan update `.env` `DB_PASSWORD`. |
| Sandi Root Server | OS Linux Debian 12 | 12 Bulan Sekali| Pemilik Usaha |
...
*   **Penambahan Akun**: Pemilik Usaha (`pemilik`) login ke menu `MENU-M7-001`, memasukkan username, sandi polos (di-hash bcrypt cost 12), dan penugasan peran default dari 8 posisi RBAC.
...
Jika terdeteksi selisih kas kasir berulang atau jumlah retur abnormal pada salah satu akun kasir:
...
2.  **Penelusuran Audit**: Ekstraksi detail `old_value` dan `new_value` transaksi retur kasir tersebut pada tabel `audit_logs`.
...
2.  Simulasikan perintah hak penghapusan data CRM pelanggan (hard delete permanently) melalui terminal menu pemilik. Pastikan transaksi historis retail yang menggunakan profil pelanggan tersebut tetap konsisten (menggunakan default cascade anonymized atau set pelanggan_id ke default 1).
...
3.  Impor file SQL tersebut ke database pusat. Constraint relational InnoDB akan menjaga data tetap utuh dan terpisah rapi berdasarkan kode `cabang_id`.
...
3.  **RAM/SSD**: Lepaskan RAM atau SSD NVMe M.2 lama, pasangkan komponen pengganti yang baru dengan spesifikasi locked (RAM 16GB DDR4, SSD NVMe 512GB).
...
3.  If Switch Hub mati total (lampu indikator padam), ganti dengan Gigabit Switch Hub 8-Port unmanaged yang baru.

### 16.4. Prosedur Penggantian Printer Thermal
Jika printer thermal nota kasir rusak fisik atau terbakar:
1.  Cabut kabel USB/Serial dan adaptor daya printer thermal yang rusak.
2.  Pasang unit thermal printer nota baru di konter kasir, hubungkan ke PC Klien.
3.  Buka setting Windows, pasang driver printer baru, dan set sebagai driver **Generic / Text Only**.
4.  Sesuaikan parameter `PRINTER_PORT` dan `PRINTER_WIDTH_MM` (58mm atau 80mm) pada berkas `.env` klien kasir.
...
| **AST-HW-007** | Router Board MikroTik hEX lite | 2026-05-20 | 5 Tahun (s.d 2031) | Pusat Kontrol Rack | Sangat Baik |
...
*   **Kemungkinan Penyebab**:
    1.  Kabel LAN Cat6 klien atau server kendor/terlepas dari port Switch Hub.
...
*   **Langkah Solusi**:
    1.  Uji ping fisik dari kasir ke server: `ping 192.168.1.200`. Jika RTO (Request Time Out), periksa colokan kabel LAN dan switch hub.
...
*   **Langkah Solusi**:
    ...
    2.  Jika pemilik usaha butuh membuka lockout instan: login SSH ke server MySQL, jalankan kueri reset attempts:
        ...
        ```
*   **Pencegahan**:
    *   Edukasi staf kasir agar mengetikkan sandi secara teliti, dan melarang penggunaan password default `seed.sql`.
...
*   **Langkah Solusi**:
    1.  Staf kasir cukup mengetikkan kembali username dan sandi polos secara valid pada prompt login CLI.
*   **Pencegahan**:
...
*   **Langkah Solusi**:
    1.  Periksa keberadaan dan hak akses file opsi:
        ```bash
        # [LINUX DEBIAN 12 — Server]
        ls -l /root/.my.cnf
        ```
...
*   **Langkah Solusi**:
    1.  Lakukan penghitungan ulang fisik kas kertas dan koin secara teliti bersama Kepala Percetakan.
    2.  Jika selisih tetap riil terjadi, panggil Kepala Percetakan untuk memasukkan sandi supervisor dan tambahkan memo anomali untuk menutup shift.
*   **Pencegahan**:
...
*   **Langkah Solusi**:
    ...
    2.  Ketik perintah manual di terminal klien kasir sebelum program Python dipicu untuk memaksa encoding UTF-8 aktif:
        ```cmd
        # [WINDOWS 11 — Klien]
        chcp 65001
        ```
...
*   **Gejala**: Kertas struk printer thermal nota keluar berupa karakter huruf acak berulang-ulang, atau laci kasir RJ11 tidak membuka otomatis.
*   **Kemungkinan Penyebab**:
...
*   **Gejala**: Layar CLI owners hang sesaat atau loading laporan laba rugi divisi M.6 memakan waktu > 2 detik (`DEF-PERF-001`).
*   **Kemungkinan Penyebab**:
    1.  Tabel transaksional `transaksi` dan `detail_transaksi` membengkak volume datanya (> 50.000 record) tanpa composite indexing.
*   **Langkah Solusi**:
...
*   **Gejala**: Stok bahan baku desimal di database MySQL berkurang tidak wajar atau tidak cocok dengan formula BOM kustom.
*   **Kemungkinan Penyebab**:
...
*   **Langkah Solusi**:
    ...
    2.  Jalankan Stock Opname fisik gudang (M.2) dan approval Kepala untuk memaksa update stok fisik riil di database.
*   **Pencegahan**:
    *   Audit standard penulisan FP Python (`01_coding_standard.md`) melarang keras sintaks float polos.

### 17.10. Akses Menu Pemilik Ditolak (ERR-AUTH-003)
*   **Gejala**: Tampil pesan `ERR-AUTH-003: Akses Ditolak: Hak Akses Pemilik Dibutuhkan!` di layar console kasir.
*   **Kemungkinan Penyebab**:
    1.  Staf dengan peran non-pemilik mencoba membuka menu khusus administratif pemilik (seperti backup manual, review payroll, edit configs, log audit).
*   **Langkah Solusi**:
    1.  Pastikan pengguna login menggunakan akun pemilik. Staf non-pemilik dilarang mengakses menu ini. Jika pemilik ingin memberikan akses, loginlah menggunakan kredensial pemilik.
*   **Pencegahan**:
    *   Terapkan dekorator RBAC secara ketat di codebase (`rbac_guard.py`) untuk memblokir navigasi menu ilegal di sisi klien.

### 17.11. Persetujuan Stock Opname Ditolak (ERR-AUTH-011)
*   **Gejala**: Tampil pesan `ERR-AUTH-011: Hak akses supervisor dibutuhkan untuk menyetujui Stock Opname!`.
*   **Kemungkinan Penyebab**:
    1.  Pengguna dengan peran `gudang` atau `pramuniaga` mencoba menyetujui (*approve*) selisih stock opname fisik.
*   **Langkah Solusi**:
    1.  Panggil `kepala_percetakan` atau `pemilik` untuk memasukkan kredensial supervisor guna menyetujui transaksi stock opname tersebut.
*   **Pencegahan**:
    *   Pastikan pembagian tugas (Segregation of Duties) diterapkan dengan baik. Persetujuan stock opname adalah wewenang supervisor.

### 17.12. Otorisasi Pengeluaran Besar Gagal (ERR-AUTH-029)
*   **Gejala**: Tampil pesan `ERR-AUTH-029: Verifikasi sandi Pemilik gagal. Pengeluaran besar dibatalkan!`.
*   **Kemungkinan Penyebab**:
    1.  Kasir menginput pengeluaran operasional &ge; Rp 500.000, lalu salah memasukkan sandi eskalasi pemilik.
*   **Langkah Solusi**:
    1.  Pastikan Pemilik Usaha (Alfatih) hadir secara fisik di konter kasir untuk mengetikkan sandi pemilik yang benar pada prompt eskalasi.
*   **Pencegahan**:
    *   Batasi pengeluaran kasir harian kecil di bawah Rp 500.000. Untuk biaya besar, pemilik harus melakukan input langsung di menu pemilik.
...
2.  **Debu Kertas Percetakan**: Debu sisa potongan kertas konter merusak sirkuit thermal kipas atau hardware klien.
3.  **Korupsi Fisik Database**: dirty shutdown kotor akibat daya UPS habis total merusak integritas index InnoDB.
4.  **Kebocoran File `.env`**: String kredensial, JWT key, dan Fernet key bocor ke staf kasir biasa.
...
| **RSK-09** | Logika Bisnis | 2 | 4 | **8** | Wajib `Decimal(15,4)` dan `ROUND_HALF_UP` di unit test pytest logic. |
...
*   Pemilik tunggal kredensial administrative root server Linux dan root MySQL.
*   Pemberi otorisasi eskalasi sandi fisik untuk retur kasir, pembatalan DP, dan pengeluaran &ge; Rp 500.000.
*   Pelaksana rotasi 6 bulanan `JWT_SECRET_KEY` dan `FERNET_KEY` di berkas `.env`.

### 19.3. Tanggung Jawab Kepala Percetakan
*   Pengawas harian unit konter laci kas kasir, absensi karyawan, dan antrian job tracking desainer.
*   Pelaksana SOP startup harian buka toko dan graceful shutdown server Debian pukul 21:05 WIB.
*   Pemberi persetujuan digital (sandi Kepala) untuk stock opname gudang M.2 dan memo handover anomali.

### 19.4. Tanggung Jawab Kasir dan Staf Operasional
*   Kasir bertanggung jawab penuh atas pencatatan mutasi laci kasir, serah terima shift harian, dan print nota struk.
*   Staf gudang bertanggung jawab atas input barang masuk supplier, input stock opname fisik, dan log limbah produksi.
*   Desainer dan staf produksi bertanggung jawab atas alur job antrian 5 status dan rekam path PDF file desain.

### 19.5. Tanggung Jawab System Administrator
*   Verifikasi kesuksesan berkas backup harian ZIP AES-256 server Debian.
*   Pelaksana bulanan `CHECK TABLE` database, purging log audit trail > 12 bulan, dan monitoring disk SSD.
*   Pemimpin prosedur restore database dan disaster recovery server baru.

### 19.6. Tanggung Jawab Tim Pengembang AI (Support)
*   Tim support luring pengembang program Python FP murni.
*   Pelaksana analisis akar masalah eror (RCA) dan pembuat branch patch hotfix Git.
...
1.  **ACID** (*Atomicity, Consistency, Isolation, Durability*): Atribut penjamin transaksi basis data MySQL InnoDB yang andal dan aman.
2.  **Audit Trail**: Catatan log kronologis terstruktur format JSON yang merekam riwayat modifikasi data sangat sensitif.
3.  **bcrypt**: Algoritma enkripsi satu arah adaptif Blowfish yang digunakan untuk menyandi kata sandi pengguna di database MySQL.
4.  **BOM** (*Bill of Materials*): Resep takaran desimal bahan baku langsung pembentuk produk kustom.
5.  **Brute-Force**: Serangan tebakan kata sandi secara berulang pada terminal laci kasir.
6.  **CLI** (*Command Line Interface*): Antarmuka aplikasi berbasis teks terminal di mana pengguna berinteraksi menggunakan keyboard tanpa mouse.
7.  **Fernet**: Kriptografi simetris dua arah dari pustaka `cryptography` Python untuk menyandi biner nomor WhatsApp pelanggan CRM.
8.  **FP** (*Functional Programming*): Paradigma penulisan kode Python murni tanpa class OOP, state global, dan mutasi biner variabel.
9.  **Fraud**: Tindakan kecurangan internal (pengambilan kas, manipulasi retur, atau edit stok ilegal).
10. **Graceful Shutdown**: Prosedur pemadaman komputer secara aman agar InnoDB engine melakukan flushing memori ke disk SSD.
11. **HPP** (*Harga Pokok Penjualan*): Nilai modal bahan baku langsung pembentuk produk cetak kustom.
12. **IEEE 14764**: Standar internasional formal yang menjabarkan siklus hidup pemeliharaan perangkat lunak (software maintenance).
13. **InnoDB**: Engine database MySQL yang mendukung constraints Foreign Key dan Repeatable-Read isolation level.
14. **ITIL** (*Information Technology Infrastructure Library*): Standar metodologi pengelolaan layanan IT yang fokus pada penyelarasan layanan IT dengan kebutuhan bisnis operasional.
15. **JWT** (*JSON Web Token*): Token stateless ter-enkripsi HS256 yang menampung identitas dan otorisasi sesi aktif staf kasir.
16. **LAN** (*Local Area Network*): Jaringan kabel lokal luring offline murni konter toko tanpa gateway internet luar.
17. **Mojibake**: Glitch rendering karakter terminal CLI box-drawing akibat kesalahan set active code page.
18. **Multi-Branch Ready**: Rancangan basis data yang menyertakan `cabang_id` di setiap tabel untuk skalabilitas ribuan cabang masa depan.
19. **MTBF** (*Mean Time Between Failures*): Indikator keandalan hardware yang mengukur rata-rata waktu operasional normal di antara dua kegagalan.
20. **MTTR** (*Mean Time To Repair*): Rata-rata waktu fisik yang dihabiskan untuk mendiagnosa, menambal, dan memulihkan kegagalan hardware/software.
21. **RBAC** (*Role-Based Access Control*): Kebijakan pembatasan hak akses menu sistem berdasarkan peran dan tanggung jawab spesifik staf.
22. **Shift Handover**: SOP serah terima laci kas kasir, pencocokan kas fisik vs sistem, dan closing sesi JWT harian.
23. **Single Source of Truth (SSoT)**: Satu-satunya acuan data atau konfigurasi tunggal yang diakui resmi di seluruh sistem.
24. **SLA** (*Service Level Agreement*): Kontrak komitmen formal yang menetapkan target tingkat layanan (MTTR / waktu respon) tanggap kendala teknis.
25. **Stock Opname**: SOP rekonsiliasi data stok persediaan fisik riil dengan catatan database sistem.
26. **UPS** (*Uninterruptible Power Supply*): Perangkat keras stabilizer daya yang menyangga daya server dan kasir &ge; 15 menit saat mati listrik.
27. **UU PDP**: Undang-Undang Perlindungan Data Pribadi No. 27 Tahun 2022 Republik Indonesia.
...
|:---:|---|---|:---:|---|
| **R-01** | Deployment Guide v1.1 | `docs/sdlc/06_deployment/01_deployment_guide.md` | **PRIMER** | Acuan utama runbook harian startup/shutdown server, backup otomatis harian, restore, dan disaster recovery. |
| **R-02** | System Architecture v1.1 | `docs/sdlc/03_design/03_system_architecture.md` | **PRIMER** | Acuan diagram deployment, node matrix, modular fungsional, 28 tabel InnoDB, connection pooling, dan retry. |
| **R-03** | Security Design v1.1 | `docs/sdlc/03_design/06_security_design.md` | **PRIMER** | Acuan hardening OS, ufw rules, bcrypt, JWT sessions, Fernet CRM, audit logs JSON, dan kamus kode error keamanan. |
| **R-04** | Environment Config v1.1 | `docs/sdlc/06_deployment/02_environment_config.yaml` | **PRIMER** | Sumber acuan parameter runtime database, enkripsi, dan driver printer thermal. |
| **R-05** | Release Notes v1.1 | `docs/sdlc/06_deployment/03_release_notes.md` | **PRIMER** | Acuan modul aktif, known limitations, known issues, dan roadmap upgrade. |
| **R-06** | Environment Setup v1.1 | `docs/sdlc/04_implementation/02_environment_setup.md` | **SEKUNDER**| Acuan kompilasi offline Python runtime, setup MySQL server, requirements, dan luring dependensi. |
| **R-07** | Bug Report Template v1.1 | `docs/sdlc/05_testing/04_bug_report_template.md` | **SEKUNDER**| Acuan severity level (S1-S5), priority (P1-P4), SLA pemulihan bug, dan format form bug report. |
| **R-08** | Test Plan v1.1 | `docs/sdlc/05_testing/01_test_plan.md` | **SEKUNDER**| Acuan regression testing, test exit criteria, code coverage, dan verification. |
| **R-09** | Coding Standard v1.1 | `docs/sdlc/04_implementation/01_coding_standard.md` | **SEKUNDER**| Acuan standar penulisan kode FP Python murni, explicit UTF-8 parameters, dan gitignore. |
| **R-10** | Git Workflow v1.1 | `docs/sdlc/04_implementation/04_git_workflow.md` | **SEKUNDER**| Acuan alur branching patch, Git commit standard, dan tagging rilis. |
...
| **R-12** | SRS v1.1 | `docs/sdlc/02_analysis/02_software_requirements.md` | **TERSIER** | Acuan spesifikasi non-fungsional performa, ketersediaan, dan portabilitas dual-OS. |
| **R-13** | Database Schema v1.1 | `docs/sdlc/03_design/01_database_schema.sql` | **TERSIER** | Acuan skema DDL fisik 28 tabel InnoDB dan constraints. |
