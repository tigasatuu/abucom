...
*   **Dokumen Input (Acuan):** Dokumen ini menyerap secara utuh visi dari [Release Notes v1.2](docs/sdlc/06_deployment/03_release_notes.md) (R-01), panduan prosedur dari [Maintenance Guide v1.2](docs/sdlc/07_maintenance/01_maintenance_guide.md) (R-02), konvensi SemVer dan commit dari [Git Workflow v1.2](docs/sdlc/04_implementation/04_git_workflow.md) (R-03), identitas dari [Project Charter v1.2](docs/sdlc/01_planning/01_project_charter.md) (R-04), dan spesifikasi dari [Software Requirements Spec v1.2](docs/sdlc/02_analysis/02_software_requirements.md) (R-05).
...
*   **System Administrator / Dukungan DevOps:** Sebagai panduan melacak perubahan konfigurasi database, port, dan dependensi.
*   **Tim Pengembang AI (Dukungan):** Sebagai pedoman menulis entri changelog baru agar tetap konsisten dengan konvensi Git.
...
*   **[SRS-F-037]** Ditambahkan panel administratif pemilik untuk memicu pencadangan basis data manual via safe subprocess `mysqldump` passwordless menjadi berkas ZIP terkompresi terenkripsi AES-256 bit. *(Modul M.2)*
*   **[SRS-F-015]** Ditambahkan pengelolaan profil data supplier, pencatatan utang usaha tempo, penghitungan tenggat jatuh tempo, dan pelunasan terintegrasi kas keluar. *(Modul M.2)*

##### M.3 — Layanan Keuangan Digital, PPOB & Jasa Service
*   **[SRS-F-016]** Ditambahkan pencatatan manual mutasi saldo virtual PPOB (Pulsa/Token) dan alert visual kasir berkedip jika saldo di bawah threshold kritis Rp 150.000. *(Modul M.3)*
*   **[SRS-F-017]** Ditambahkan fitur komparasi biaya admin asli dari 6 e-wallet (Mandiri Agen, Dana, Gopay, LinkAja, ShopeePay, OVO) untuk menyarankan pengiriman paling ekonomis bagi pelanggan dan penghitungan komisi toko. *(Modul M.3)*
*   **[SRS-F-018]** Ditambahkan pencatatan transaksi jasa service & perbaikan PC/printer, terintegrasi dengan pemotongan stok suku cadang gudang dan pendebitan biaya suku cadang ke nota service. *(Modul M.3)*

##### M.4 — Manajemen SDM, Penggajian & Poin Karyawan
*   **[SRS-F-019]** Ditambahkan pengelolaan basis data karyawan, pencatatan absensi shift harian, dan pengajuan kasbon staf (limit kasbon aktif Rp 1.000.000). *(Modul M.4)*
*   **[SRS-F-020]** Ditambahkan komputasi payroll otomatis berbasis jaminan jaring pengaman minimum 50% UMR (Rp 1.600.000) dengan upah dinamis: Skenario A (gaji tetap jika target laba bersih toko Rp 15.000.000 tercapai) atau Skenario B (alokasi 25% dari laba berjalan dibagi proporsional kehadiran jika target tidak tercapai). *(Modul M.4)*
*   **[SRS-F-021]** Ditambahkan sistem poin insentif otomatis berbasis beban kerja 4-tier transaksi staf harian (Tier 1: Rp 500/poin, Tier 2: Rp 1.500/poin, Tier 3: Rp 2.500/poin, Tier 4: Rp 5.000/poin). *(Modul M.4)*
*   **[SRS-F-022]** Ditambahkan pemotongan gaji bersih bulanan secara otomatis jika terdeteksi memiliki utang kasbon aktif, dan mengubah status kasbon menjadi LUNAS. *(Modul M.4)*

##### M.5 — Sistem Manajemen Antrian & Pelacakan Desain
*   **[SRS-F-023]** Ditambahkan dashboard antrian pekerjaan sekuensial real-time 5 status (`Antri`, `Proses Desain`, `Produksi`, `Selesai`, `Diambil`) per divisi staf. *(Modul M.5)*
*   **[SRS-F-024]** Ditambahkan penyimpanan string path lokasi mockup desain PDF pelanggan di server lokal, kompatibel Dual-OS (`pathlib` Windows/Linux). *(Modul M.5)*
*   **[SRS-F-025]** Ditambahkan pembuatan string pesan notifikasi WhatsApp otomatis berisi invoice, nama pelanggan, sisa DP terformat dengan link wa.me URL encoded (`urllib.parse.quote`). *(Modul M.5)*

##### M.6 — Administrasi Pinjaman, Aset & Pengeluaran
*   **[SRS-F-026]** Ditambahkan pencatatan pinjaman modal terstruktur, membedakan pinjaman komersial bank berbunga (BRI/Mandiri) lengkap dengan tenor jatuh tempo dan pinjaman kerabat tanpa bunga yang fleksibel. *(Modul M.6)*
*   **[SRS-F-027]** Ditambahkan kompilasi laporan Laba/Rugi komprehensif instan per divisi (pendapatan, HPP desimal, OPEX, biaya limbah, penyusutan aset) selesai dalam waktu < 2.0 detik. *(Modul M.6)*
*   **[SRS-F-028]** Ditambahkan notifikasi peringatan visual kuning berkedip saat startup login pemilik jika sisa hari jatuh tempo cicilan bank/utang supplier &le; H-3 (berkedip merah jika lewat jatuh tempo). *(Modul M.6)*
*   **[SRS-F-029]** Ditambahkan pengelolaan aset tetap, penghitungan depresiasi bulanan garis lurus sebagai OPEX, dan alokasi tabungan virtual mesin baru. *(Modul M.6)*
*   **[SRS-F-030]** Ditambahkan pencatatan pengeluaran operasional rutin bulanan dan biaya tak terduga, dengan pengetatan otorisasi verifikasi sandi pemilik fisik untuk pengeluaran &ge; Rp 500.000. *(Modul M.6)*

##### M.7 — Keamanan, Hak Akses & Audit Trail
*   **[SRS-F-031]** Ditambahkan proteksi otorisasi Role-Based Access Control (RBAC) 8 peran berbasis token JWT HS256. *(Modul M.7)*
*   **[SRS-F-032]** Ditambahkan audit trail kronologis otomatis dalam format JSON untuk merekam perubahan data sensitif (`timestamp`, `user_id`, aksi, nama tabel, `old_value`, `new_value`). *(Modul M.7)*
*   **[SRS-F-033]** Ditambahkan log serah terima shift karyawan (*closing shift*) dengan input kas laci fisik dan penguncian baris transaksi shift lama dari modifikasi. *(Modul M.7)*
*   **[SRS-F-034]** Ditambahkan rekonsiliasi kas harian kasir untuk membandingkan uang kas laci fisik vs sistem, dengan alert anomali fraud jika selisih > Rp 10.000. *(Modul M.7)*
*   **[SRS-F-035]** Ditambahkan sistem peringatan anomali transaksi (*fraud detection* sederhana) berupa tanda visual merah jika gagal login brute force > 5x, pembatalan DP > 3x, atau selisih kas > Rp 10.000. *(Modul M.7)*
*   **[SRS-F-036]** Ditambahkan antarmuka menu setup awal wizard inisialisasi master data transaksional dari spreadsheet saat deployment. *(Modul M.7)*

##### M.8 — Pembatalan, Retur & CRM
*   **[SRS-F-038]** Ditambahkan basis data pelanggan terstruktur (CRM sederhana) dengan WhatsApp terenkripsi Fernet, mematuhi UU PDP No. 27/2022 (mendukung *Right to Erasure* / hard delete permanen). *(Modul M.8)*

##### M.9 — Skalabilitas Multi-Cabang
*   **[SRS-F-039]** Ditambahkan arsitektur data multi-cabang (*Multi-Branch Ready*) dengan menyematkan kolom kunci asing `cabang_id` (INT) di setiap tabel basis data dan default filtering query SELECT. *(Modul M.9)*

##### M.10 — Konfigurasi Sistem Runtime
*   **[SRS-F-040]** Ditambahkan sistem konfigurasi dinamis tanpa hardcode berbasis tabel `system_configs` di MySQL (untuk target laba, limit kasbon, threshold PPOB, toleransi kasir, rupiah per poin, UMR daerah). *(Modul M.10)*
...
*   **Enkripsi Data Pribadi Pelanggan:** WhatsApp pelanggan disandi biner **Fernet** dengan kunci 32-byte Base64 sebelum disimpan ke tabel `pelanggan.whatsapp`, mematuhi **UU PDP No. 27 Tahun 2022**.
*   **Right to Erasure CRM:** Penghapusan profil data WhatsApp secara permanen (*hard delete*) atas permintaan lisan pelanggan difasilitasi di basis data secara atomik.
...
*   **Pencadangan Terenkripsi AES-256 ZIP:** Skrip cron harian pukul 21:00 WIB mengekspor basis data transaksional secara passwordless via `/root/.my.cnf` (600), dikompres ZIP terenkripsi algoritma **AES-256 bit**.
...
*   **Mesin Basis Data:** Database MySQL 8.4 LTS menggunakan engine transaksional InnoDB (28 tabel relasional ACID compliant), isolation level REPEATABLE READ.
...
*   **Locked Dependencies (requirements.txt):** Driver `mysql-connector-python==8.4.0`, dotenv `python-dotenv==1.0.1`, hashing `bcrypt==4.1.0`, token `pyjwt==2.8.0`, visual `rich==13.7.0`, dan tabular `tabulate==0.9.0`.
*   **Pengumpulan Koneksi (Connection Pooling):** Manajemen reuse pool koneksi basis data (maksimal size 5) dilengkapi auto-retry 3x exponential backoff untuk query LAN.
...
*   **Runtime Version Kritis:** Program kasir wajib dijalankan minimal pada Python versi 3.14.2+ dengan basis data relasional MySQL lokal (tidak mendukung SQLite produksi).
...
Penyusunan berkas Changelog v1.2 didasarkan secara mutlak pada 14 berkas dokumentasi formal SDLC AbuCom:

| Kode Ref | Nama Dokumen Referensi | Path Relatif Berkas | Versi | Prioritas | Peran / Hubungan dalam Penyusunan |
|:---:|---|---|:---:|:---:|---|
| **R-01** | Release Notes v1.2 | `docs/sdlc/06_deployment/03_release_notes.md` | 1.2 | **PRIMER** | Acuan utama seluruh fitur baru M.1 s.d M.10, keamanan, bug fixes, known limitations, known issues, roadmap, dan dependencies. |
| **R-02** | Maintenance Guide v1.2 | `docs/sdlc/07_maintenance/01_maintenance_guide.md` | 1.2 | **PRIMER** | Acuan daftar modul aktif, 28 tabel basis data InnoDB, locked libraries, dan kategori pemeliharaan sistem. |
| **R-03** | Git Workflow v1.2 | `docs/sdlc/04_implementation/04_git_workflow.md` | 1.2 | **PRIMER** | Acuan standardisasi SemVer, Conventional Commits, tagging, dan pembagian tanggung jawab tim campuran. |
| **R-04** | Project Charter v1.2 | `docs/sdlc/01_planning/01_project_charter.md` | 1.2 | **SEKUNDER** | Acuan nama proyek, deskripsi, tim pengembang, milestone, dan informasi sponsor. |
| **R-05** | Software Requirements Spec v1.2 | `docs/sdlc/02_analysis/02_software_requirements.md` | 1.2 | **SEKUNDER** | Acuan pemetaan kode kebutuhan fungsional (SRS-F-001 s.d SRS-F-040) ke entri changelog. |
| **R-06** | Database Schema v1.2 | `docs/sdlc/03_design/01_database_schema.sql` | 1.2 | **SEKUNDER** | Acuan verifikasi 28 nama tabel InnoDB untuk kategori Infrastruktur. |
| **R-07** | System Architecture v1.2 | `docs/sdlc/03_design/03_system_architecture.md` | 1.2 | **SEKUNDER** | Acuan diagram deployment, topologi LAN luring, hardware specs, dan connection pooling. |
| **R-08** | Security Design v1.2 | `docs/sdlc/03_design/06_security_design.md` | 1.2 | **SEKUNDER** | Acuan spesifikasi otentikasi bcrypt, sesi JWT, Fernet CRM, AES-256 backup, dan audit trail JSON. |
| **R-09** | Coding Standard v1.2 | `docs/sdlc/04_implementation/01_coding_standard.md` | 1.2 | **TERSIER** | Acuan standardisasi FP murni, type hints, PEP 8/257, dan presisi Decimal. |
| **R-10** | Module Structure v1.2 | `docs/sdlc/04_implementation/03_module_structure.md` | 1.2 | **TERSIER** | Acuan pemetaan file-to-module untuk scope referensi changelog. |
| **R-11** | Test Plan v1.2 | `docs/sdlc/05_testing/01_test_plan.md` | 1.2 | **TERSIER** | Acuan hasil testing, exit criteria, dan QA summary. |
| **R-12** | Deployment Guide v1.2 | `docs/sdlc/06_deployment/01_deployment_guide.md` | 1.2 | **TERSIER** | Acuan prosedur instalasi offline, setup database, dan runbook pemeliharaan harian. |
| **R-13** | Tech Stack Decision v1.2 | `docs/sdlc/01_planning/04_tech_stack_decision.md` | 1.2 | **TERSIER** | Acuan locked packages, batasan platform runtime Python 3.14.2+ & MySQL 8.4. |
| **R-14** | Bug Report Template v1.2 | `docs/sdlc/05_testing/04_bug_report_template.md` | 1.2 | **TERSIER** | Acuan format bug severity, priority level, dan known issues. |
