...
- [x] **1.1** Buka dan baca dokumen utama secara lengkap dari baris pertama hingga terakhir:
...
- [x] **1.2** Buka dan baca dokumen referensi **BRD v1.1** secara lengkap:
...
- [x] **1.3** Buka dan baca dokumen referensi **SRS v1.1** secara lengkap:
...
- [x] **1.4** Buka dan baca dokumen referensi **Use Case Diagram v1.1**:
...
- [x] **1.5** Buka dan baca dokumen referensi **Workflow Diagram v1.1**:
...
- [x] **1.6** Buka dan baca dokumen referensi **Tech Stack Decision v1.1**:
...
- [x] **1.7** Buka dan baca dokumen referensi **Stakeholder Register v1.1**:
...
- [x] **1.8** Buka dan baca dokumen referensi **Feasibility Study v1.1**:
...
- [x] **1.9** Buka dan baca dokumen referensi **Innovation Proposal v1.1** dan **Project Charter v1.1**:
...
- [x] **2.1** **Validasi Kelengkapan Daftar Tabel (Bab 2.1)**:
...
- [x] **2.2** **Validasi Kelengkapan Entitas dari SRS**:
...
- [x] **2.3** **Validasi Kelengkapan Atribut Tiap Tabel vs SRS/BRD**:
...
- [x] **2.4** **Validasi Domain Nilai (Bab 4) vs Referensi**:
...
- [x] **2.5** **Validasi Matriks FK (Bab 5.1) vs Spesifikasi Tabel (Bab 3)**:
...
- [x] **2.6** **Validasi ERD Mermaid (Bab 2.2) vs Matriks FK (Bab 5.1)**:
...
- [x] **2.7** **Validasi Aturan Bisnis (Bab 6) vs SRS/BRD**:
...
- [x] **2.8** **Validasi Matriks Traceability (Bab 8) vs Referensi**:
...
- [x] **2.9** **Validasi Glosarium (Bab 9) vs Isi Dokumen**:
...
- [x] **2.10** **Validasi Referensi Dokumen (Bab 10) vs File Aktual**:
...
- [x] **3.1** **Cek Data/Informasi yang Tidak Relevan**:
...
- [x] **3.3** **Cek Kelengkapan Informasi Spesifik Data Dictionary**:
  - Periksa apakah ada elemen standar sebuah Data Dictionary profesional yang **belum ada** di dokumen ini, yaitu:
    - [x] Apakah ada **Composite Unique Constraint** yang perlu didefinisikan secara eksplisit? (Misalnya: `absensi(pengguna_id, tanggal)` harusnya UNIQUE composite — sudah ada di indeks Bab 7.3, tapi apakah sudah didefinisikan sebagai constraint di spesifikasi tabel Bab 3.10?)
    - [x] Apakah ada **CHECK Constraint** MySQL yang perlu didefinisikan? (Misalnya: `gaji_bersih >= 0`, `stok_saat_ini >= -batas_tertentu`, `failed_login_attempts BETWEEN 0 AND 5`)
    - [x] Apakah **Composite Index** pada Bab 7.3 sudah cukup atau ada index tambahan yang dibutuhkan berdasarkan pola query di SRS?
    - [x] Apakah ada **View** atau **Stored Procedure** yang direncanakan? (Jika ada di SRS, dokumentasikan sebagai catatan di Data Dictionary)
    - [x] Apakah **seed data awal** (data contoh/dummy) untuk tabel `system_configs` dan `cabang` sudah tertuang atau setidaknya direferensikan?

---

### FASE 4 — VALIDASI STANDAR STRUKTUR DOKUMEN

- [x] **4.1** **Cek Kelengkapan Header Dokumen**:
...
- [x] **4.2** **Cek Konsistensi Struktur Bab**:
  - Periksa apakah setiap spesifikasi tabel di Bab 3 memiliki struktur yang konsisten:
    - [x] Tabel metadata tabel (Nama Tabel, Deskripsi, Modul Terkait, Derivasi SRS, Derivasi BRD, Engine, Charset, Collation).
    - [x] Heading `#### Tabel Detail Atribut/Kolom`.
    - [x] Tabel kolom dengan header: `No | Nama Kolom | Tipe Data MySQL | Constraint | Null? | Default | Deskripsi Bisnis | Domain Nilai`.
    - [x] Catatan Implementasi (jika relevan).
...
- [x] **4.4** **Cek Konvensi Penamaan (Bab 1.6)**:
...
- [x] **4.5** **Cek Kelengkapan Bab Wajib Dokumen Data Dictionary**:
  - Periksa apakah dokumen memiliki minimal bab-bab berikut yang wajib ada dalam Data Dictionary standar industri:
    - [x] Informasi Dokumen (Tujuan, Cakupan, Posisi SDLC, Audiens, Konvensi) ✓
    - [x] Ringkasan Model Data (Daftar Entitas, ERD, Statistik) ✓
    - [x] Spesifikasi Entitas & Atribut Detail ✓
    - [x] Kamus Domain Nilai ✓
    - [x] Matriks Relasi FK & Constraint Integritas ✓
    - [x] Aturan Bisnis Data ✓
    - [x] Indeks & Optimasi ✓
    - [x] Matriks Traceability ✓
    - [x] Glosarium ✓
    - [x] Referensi Dokumen ✓
    - [x] **Apakah ada bab yang masih diperlukan tetapi belum ada?** (Misalnya: Rencana Migrasi Data Awal / Seed Data, atau Kebijakan Retensi Data)

---

### FASE 5 — VALIDASI KELAYAKAN SEBAGAI INPUT FASE 03 DESIGN

- [x] **5.1** **Cek Kelengkapan Informasi untuk Penulisan `schema.sql`**:
  ...
  - Periksa untuk setiap tabel:
    - [x] Apakah nama tabel jelas?
    - [x] Apakah tipe data setiap kolom sudah ditentukan secara spesifik? (Tidak ada yang hanya ditulis "INT" tanpa konteks UNSIGNED atau signed)
    - [x] Apakah constraint setiap kolom sudah lengkap (PK, FK, UQ, NN, AI, DEFAULT)?
    - [x] Apakah nilai DEFAULT sudah ditentukan untuk setiap kolom yang memiliki default?
    - [x] Apakah perilaku ON DELETE dan ON UPDATE setiap FK sudah terdefinisi di Matriks FK Bab 5.1?
    - [x] Apakah Engine, Charset, dan Collation sudah didefinisikan per tabel?
    - [x] Apakah Composite Unique Constraint (misal pada `absensi`) sudah terdefinisi secara eksplisit?

- [x] **5.2** **Cek Kelengkapan Informasi untuk Penulisan `seed.sql`**:
...
- [x] **5.3** **Cek Potensi Pertanyaan yang Akan Mengganggu Proses Fase 03**:
  - Simulasikan: Bayangkan seorang programmer sedang membuat `schema.sql` berdasarkan dokumen ini dan menemukan kebingungan. Apakah ada hal-hal berikut yang masih tidak jelas?
    - [x] Apakah urutan pembuatan tabel (tabel parent sebelum child) bisa dipahami dari ERD?
    - [x] Apakah ada circular dependency antara tabel yang bisa menyebabkan masalah saat eksekusi DDL?
    - [x] Apakah kolom `metode_pembayaran` pada transaksi akan memengaruhi rekonsiliasi kas di `shift_handover`? Apakah relasinya sudah jelas?
    - [x] Bagaimana cara membedakan `barang` bertipe `Retail_ATK` vs `Bahan_Baku` secara programatik — apakah sudah cukup jelas hanya dengan `tipe_barang`?

---

### FASE 6 — VALIDASI KUALITAS BAHASA INDONESIA

- [x] **6.1** **Cek Kejelasan Deskripsi Bisnis Kolom**:
  - Baca ulang kolom `Deskripsi Bisnis` di setiap tabel Bab 3. Apakah deskripsi sudah:
    - [x] Menjelaskan **fungsi bisnis** kolom tersebut (bukan hanya nama teknis)?
    - [x] Tidak ambigu (tidak menimbulkan tafsiran ganda)?
    - [x] Menggunakan bahasa Indonesia yang natural dan baku?
    - [x] Cukup informatif sehingga junior programmer atau AI model bisa memahami konteks penggunaan kolom tersebut?
  - Tandai kolom yang deskripsinya masih terlalu teknis, terlalu singkat, atau ambigu.

- [x] **6.2** **Cek Konsistensi Terminologi**:
...
- [x] **6.3** **Cek Catatan Implementasi**:
  - Baca setiap Catatan Implementasi di Bab 3. Apakah:
    - [x] Catatannya relevan dan spesifik untuk tabel tersebut?
    - [x] Bahasanya jelas dan actionable untuk developer?
    - [x] Tidak bertentangan dengan informasi di SRS atau BRD?

- [x] **6.4** **Cek Deskripsi Bab Pembuka**:
  - Baca Bab 1 (Informasi Dokumen). Apakah:
    - [x] Tujuan dokumen sudah jelas dan spesifik?
    - [x] Cakupan dokumen sudah akurat (28 tabel, 18 domain nilai, 103 FK — apakah angka ini akurat?)?
    - [x] Posisi dokumen dalam SDLC sudah benar dan informatif?
    - [x] Daftar audiens target sudah lengkap dan relevan?

---

### FASE 7 — VALIDASI DATA KOSONG & PLACEHOLDER

- [x] **7.1** **Cek Kolom Default Kosong atau Tidak Tersedia**:
...
- [x] **7.3** **Cek Referensi yang Hilang (Broken References)**:
...
- [x] **7.4** **Cek Data Spesifik Tabel yang Berpotensi Kosong**:
...
- [x] **8.1** **Cek Normalisasi Database (1NF, 2NF, 3NF)**:
...
- [x] **8.2** **Cek Konsistensi Tipe Data antar Kolom yang Saling Merujuk**:
...
- [x] **8.3** **Cek Potensi Masalah Performa dan Skalabilitas**:
...
- [x] **8.4** **Cek Konsistensi Kebijakan `updated_at` di Tabel Audit-Sensitif**:
...
- [x] **8.5** **Cek Kelengkapan Keterangan Derivasi Tabel**:
...
- [x] **8.6** **Cek Completeness Statistik (Bab 2.3)**:
  - Klaim di Bab 2.3:
    - Total tabel: 28
...
- [x] **9.1** **Kompilasi Seluruh Temuan**:
...
- [x] **10.1** **Tulis Ulang Seluruh Dokumen ke Target File**:
...
- [x] **10.3** **Validasi Hasil Penulisan Ulang**:
  - Setelah file berhasil ditulis ulang, buka kembali file tersebut dan verifikasi:
    - [x] Versi sudah berubah menjadi `1.1`.
    - [x] Tabel Riwayat Perubahan sudah ada entri baru untuk v1.1.
    - [x] Seluruh bab dari Bab 1 hingga Bab 10 masih ada dan tidak ada yang terpotong.
    - [x] Semua temuan KRITIS dan PENTING sudah diperbaiki dalam dokumen baru.
    - [x] Tidak ada teks `...`, `dst.`, `[lanjutan]`, atau indikasi truncation apapun.
    - [x] Jika ada referensi dokumen baru yang ditambahkan dalam proses perbaikan, pastikan sudah tercantum di bagian akhir Bab 10.

- [x] **10.4** **Tambahkan Referensi Baru (Jika Ada)**:
